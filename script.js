...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentContentType = 'all'; // 'all', 'anime', या 'comic'

    // --- 3. API से बात करने वाले फंक्शन्स ---
    
    /**
     * काउंटर (व्यू/क्लिक) को अपडेट करता है। (आपके ओरिजिनल कोड से)
     */
    const updateCounter = async (postId, type) => {
        ...
        const trySendBeacon = () => {
            ...
            } catch (e) { console.warn('sendBeacon error', e); }
            return false;
        };
        ...
        try {
            ...
            if (res.status === 405 || res.status === 404) {
                ...
                const res2 = await fetch(url, {
                    method: 'PATCH', mode: 'cors', keepalive: true,
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ _id: postId, type })
                });
                if (res2.ok) { console.debug('updateCounter: PATCH ok for', url); return; }
                else { console.warn('PATCH also failed', res2.status, res2.statusText); }
            } else {
            ...
            try { if (navigator.sendBeacon) navigator.sendBeacon(url); }
            catch (e) { console.warn('final sendBeacon also failed', e); }
        }
    };
            ...
            <div class="card-content">
                ...
                <div class="card-footer">
                    ...
                    <a href="${post.link}" class="card-link" target="_blank" rel="noopener noreferrer">
                        ${post.postType === 'comic' ? 'Read Now' : 'Watch Now'}
                    </a>
                </div>
            </div>
        `;
        const link = card.querySelector('.card-link');
        const viewsNum = card.querySelector(`#views-${post._id}`);
    ...
    const fetchPaginatedPosts = async (page = 1) => {
        ...
        try {
            ...
            const typeText = currentContentType === 'anime' ? 'Anime' : currentContentType === 'comic' ? 'Comics' : 'Posts';
            gridTitle.textContent = `Latest ${typeText} (Page ${data.currentPage})`;
        } catch (error) {
            animeGrid.innerHTML = '<p class="info-message">Failed to load posts. Please try again later.</p>';
    ...
    /**
     * सर्वर पर किसी शब्द को खोजता है।
     */
    const fetchSearchResults = async (term) => {
        ...
        try {
            ...
            animeGrid.innerHTML = '<p class="info-message">Search failed. Please try again.</p>';
    ...
    const loadTags = (posts) => {
        ...
        posts.forEach(post => {
            if (post.category) {
                post.category.split(',').map(tag => tag.trim()).filter(Boolean).forEach(tag => tagSet.add(tag));
            }
            if (post.tags && Array.isArray(post.tags)) {
                post.tags.forEach(tag => { if (tag.trim()) tagSet.add(tag.trim()); });
            }
        });
    ...
    const renderPosts = (posts) => {
        ...
        if (!posts || posts.length === 0) {
            animeGrid.innerHTML = '<p class="info-message">No posts found.</p>';
            return;
        }
        posts.forEach(post => {
            ...
            card.addEventListener('click', (e) => {
                ...
                try { window.open(post.link, '_blank', 'noopener,noreferrer'); }
                catch (ex) { window.location.href = post.link; }
